...
print(f'Медиана массива через statistics: {median(random_list)}')
...
'''
Аналитика: Встроенная функция медианы модуля статистикс оказалась самой быстрой. Превосходит
остальные функции в десятки раз. На больших значениях m разница становится еще более ощутимой. Скорей всего встроенные
функции для того и созданы, чтоб не изобретать велосипед и пользоваться ими получаея преимучество в быстродейсвии
реализованное по капотом функции.
Поиск меданы через сортировку Шелла на малых m < 50, проигрывает в быстродействии поиску без сортировки. При больших значениях
m поиск без сортировки оказывается существенно медленнее Шеллы.
'''